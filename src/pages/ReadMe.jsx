...
- **\`OWNERSHIP_QUESTION\` is conditional, and the intro has to stay conditional with it.** The question is only asked when the assessment names roles, so \`IntroPurpose\` takes \`showOwnership\` and drops both the list item and the "first two" in its explanatory line when there are none — describing a question that never appears is worse than leaving it undescribed. Anything added to the intro that names a question must ask the same thing about when that question renders.
...
- Nothing in the app calls a suggested owner an owner. The field is \`suggested_owner\`, the respondent's column reads **Suggested owner**, and the team tally reads **Most suggested** — a respondent proposes a role, and no part of the assessment assigns one. The survey now says so where it asks: the hint reads *the role that should be accountable, not who does it today*, because the failure mode is a respondent describing the status quo and producing a tidy answer that reports what the room already knows.`,
  },
];
