...
| **ActivitySet** | Named presets of activities for quick assessment setup. \`description\` says what the set is for, and shows wherever a preset is picked. \`content_key\` is its id in \`content/activity-sets.md\`, which is where a set survives the app. |
...
| **Instrument** | One of the seven things an assessment can run: its name, tagline, two descriptions, sections, which scales it uses, whether it asks about ownership, its report style, and the label for its \`subject\` where it has one. \`description\` is what the person answering reads at the top of the survey, and \`summary\` is what a facilitator reads when choosing between them in the New Assessment panel — two audiences, so two fields. \`key\` is stable and is what its content file is matched on. \`band_basis\` says whether its bands are measured in points earned or in the average answer. \`internal\` keeps it out of the New Assessment panel for everyone but a super-admin — presentation, not protection, since instrument content reads open so the survey can render it. |
| **Scale** / **ScaleOption** | The answer scales an instrument offers and the options on each, with the treatment for "I don't know". |
| **Band** | A score range on an instrument, with the advice written for landing in it. Only the instruments that carry a verdict have any. Read against a points total or against the average answer, depending on the instrument's \`band_basis\`; on a mean the bands are a ladder of floors, because inclusive bounds cannot tile a continuous range without leaving a hairline nobody could fall into. |
| **InstrumentSection** | One dimension of a dimension-report instrument: what it is about, and the two paragraphs read when it is somebody's strongest or their weakest. Its \`name\` is the join to the section its questions carry and to the entry in \`Instrument.sections\`, so renaming a dimension means renaming it in all three. |
...
| **SkippedPost** | A blog post deliberately not made a resource, so the Resources panel stops offering it. \`content_key\` is its id in \`content/skipped-posts.md\`, which is the article's slug. |
...
\`publicAssessment\` resolves every public token server-side and returns only the fields that flow needs, plus \`org_name\` — the firm the printed report names, resolved from the assessment's organisation or its creator's, since the pages that render it are unauthenticated and cannot read \`Organization\` themselves · \`saveResponses\` upserts a page of answers by activity against the token, and carries the wrap-up's free text and the completion flag in the same call · \`listRespondents\` and \`listUsers\` read as service role where RLS cannot express the rule · \`summarizeRespondents\` returns respondent counts and completion times for every assessment the caller can see, with no names, so the Assessments page can show progress and unread badges in one call · \`deleteAssessment\` cascades a delete after one authority check · \`deleteOrganization\`, \`deleteTeamMember\`, \`deleteTag\` and \`deleteLibraryActivity\` do the reverse, refusing while anything still references the row · \`listTags\` and \`listLibraryActivityUsage\` count usage across records the caller cannot read, so a Delete is only offered where it can work · \`acceptInvitation\` and \`updateTeamMember\` manage roles server-side — the latter refuses every self-edit except one, a super-admin setting their own organisation, whose access does not derive from it.`,
  },
  {
    id: "architecture",
    group: null,
...
**An export has to mean everything, and for a while it did not.** The old Download content wrote a section as a bare name while the prose attached to it — a dimension's blurb, and the two paragraphs read when it is somebody's strongest or weakest — was left out entirely: ten of the practice profile's fifteen prose fields missing from its own backup, in a file that looked complete. The cause was two hand-written serializers of one thing, the seed and the export, kept in agreement by memory. There is one field map now, in \`content-format.js\`, read by the writer and the reader both, and \`npm test\` sets every field in it to a distinctive value and insists it survives the round trip. The same test refuses a file the app cannot write back out identically.

**The Fractional CPO Practice Profile is the one instrument we run on our own prospects.** Fifteen statements over five dimensions, on a four-option scale, banded on the average answer rather than on a total. It is \`internal\`, so it never appears in a customer organization's New Assessment panel, and its report is the only one in the app that draws a per-dimension profile: five bars, the strongest dimension, the one with the most leverage in it, and prose written for each. That report is read on the respondent's own copy, because it describes one practice. Run several practitioners through one standing assessment and the ordinary distribution report answers the other question — what share of fractional CPOs say they have a repeatable diagnosis — which is why the intro promises aggregate-only reporting in as many words.

**A four-option scale, where the document asked for five.** Every rating scale in this app has four options and no midpoint, which forces a lean: Inconsistent or Good, Usually or Rarely. A midpoint on a fifteen-statement instrument risks a flat profile — no strongest area, no leverage opportunity, and nothing for the reader to act on — and the four-option form keeps this scale consistent with the other eight.
...
**A field with a documented default is absent on the oldest records, not set to it.** \`assessment_type\` was added after assessments already existed, so \`team_gap\` is what the schema *means* by an empty column, never what it stores there. Any \`=== "team_gap"\` test therefore misses exactly the earliest records — they fall through to whatever the else branch does. Testing for \`personal\` has always been safe, which is why nothing had tripped on this until the sidebar needed a positive test for team gap; written the obvious way it would have left the oldest assessments unbadged, the very gap the badge was added to close. Don't test the field at all: ask \`kindOf(assessment, instrument)\` in \`instrument-kind.js\`, which returns \`team_gap\`, \`personal\`, or \`own_questions\`. It lets the instrument decide and reads \`assessment_type\` only when there is none, and the third kind is the reason. An own-questions assessment carries no \`assessment_type\`, so "not personal" also matched Chaos and Portfolio Health, and until September 2026 they were offered as the team side a personal assessment could link to. The same applies to \`org_id\`, whose absence means the legacy no-org bucket.
...
**A heading can be the last thing that fits, and only one browser will show you.** The practice profile printed from iOS Safari with ALIGN alone at the foot of page four and DEMONSTRATE alone at the foot of page seven, each announcing a section that began overleaf. \`break-after: avoid\` on the heading is the obvious fix and WebKit has never honoured it reliably; what works is wrapping the heading and the first item under it in one block that may not be split, with the legacy \`page-break-inside\` alias alongside the modern property, since Tailwind's utility emits only the latter. Headless Chrome could not reproduce any of it — it paginated the same report in seven pages where iOS took eleven — so **a print check that only runs in Chrome is not a print check**. Save the PDF from Safari on a simulator and read it.

**Presence is not placement.** Extracting a printed PDF's text and asserting every section, question and paragraph is there passes cleanly while two headings sit orphaned at the foot of a page. A text check answers "is it all here", which is worth asking and is not the question print raises. Check what each page *ends* on as well: a page whose last line is a bare heading is the defect, and it is the one a person spots immediately and a completeness assertion never will.
...
**A fixed-height cover can make iOS drop the end of a report.** The printed cover used to fill its sheet so the credit sat at the foot, sized by arithmetic about the printable area — which every browser answers differently. On an iPhone the printable area on Letter is about 8in, so the credit overran onto page two; shortening the cover to fit then made iOS Safari count one page too few, and the saved PDF ended partway through the appendix, with nothing marking the loss. Measured in the iOS Simulator, the only arrangement that printed complete everywhere was a cover sized by its content, with the credit directly under the date. **After any print change, save a PDF from iOS Safari and read the last page.** The QA print check runs in Chrome, which paginates correctly, so it cannot see this.

**Safari ignores \`break-after: avoid\`.** Chrome honours it, so every heading pinned that way looked right in the QA sweep and still printed alone at the foot of a page on an iPhone — a phase label with its activities overleaf, "Part four · Suggested resources" above a page break, a card holding only its key line. Every engine honours \`break-inside: avoid\`, so the personal report now builds those pairings from it: a phase label and its first activity, a section heading and its first card (\`withHeading\`), the bar key and its first phase, the appendix heading and each phase table short enough to fit a sheet (\`ActivityAnswerTable\`, up to ten rows). Pair with the *first* item only, or a long section jumps to a new page whole. The QA print check runs in Chrome and cannot see this class of bug; only a PDF saved on an iPhone can.
...
- **Reading lives at two addresses, and only one comes from a content file.** Blog articles point at \`/reading/<slug>\`, the CTA-free copy composed from \`ARTICLE_PREFIX\` in content-apply.js. The 23 chaos limericks point at the Wix dynamic page \`/reading-idiom/<slug>\` (it was \`/i/<slug>\` until September 2026), were added in the app, and are in no content file, so applying one leaves them alone. Apostrophes are dropped from the slug (\`whos-on-first\`). Wix's \`dynamic-i_…-sitemap.xml\` may still list the old \`/i/\` addresses and apostrophe slugs, so check a page loads before trusting it.
- **Applying a content file never overwrites a library resource.** For an article already in Settings → Resources, it sets only the address (the /reading/ policy) and the links its own questions own. Title, type, note, order, fallback, and active stay as the library set them, so an article switched off there stays off, and a link removed from a file removes only that question's link. A new article gets every field.
- **Delete is locked, on every assessment, every time.** The delete control sits behind a lock that starts closed whenever the screen is opened, and closes again the moment another assessment is selected. There was already a confirmation, and a confirmation is a thing you click through — it arrives under the cursor that was already moving. The lock is a separate act on a separate control, so it cannot be satisfied by the reflex that started the delete. It is not a stored flag: a flag has to be remembered when the assessment is made, and the one deleted by accident is the one nobody thought to protect.
- **A job title rename does not cascade, and the sync says so before it happens.** \`Activity.preferred_owner\` and \`Response.suggested_owner\` both hold the name as text, so renaming a title leaves activities recommending a function that no longer exists and answers naming one nobody can pick. Syncing \`content/job-titles.md\` counts the activities still pointing at the old name and says that answers keep it, then writes only the title.
- **The activity library and the reading are content files too.** The library is a file per phase in \`content/library/\`, where the phase is which file an activity is in and the order is where it sits in that file; \`content/resources.md\` holds each article's record and the library activities it is offered for. An instrument question's reading stays in that instrument's file, because one link declared in two places is the drift this format exists to remove. Neither is deleted from the app by a sync: assessments, activity sets, answers, and workshop notes all point at library activities, so one dropped from the files is reported and left alone. The point of having them here at all is that the repository should still hold the backbone of the app if it ever leaves Base44.
- **The activity sets are in \`content/activity-sets.md\`, and that file is their only backup.** A set is a hand-picked list of library activities with a name and a note saying what it is for; nothing else in the app reproduces one, so losing a row means choosing it again. The file names each member by its library \`content_key\`, which means the library has to be applied before the sets — a member whose id does not resolve is named on the screen and left out, never written away. Membership is a set and not a sequence, so the file writes the members in library order and reshuffling in the app is not a difference. Nothing is deleted either way: a set dropped from the file is reported, and a set the file has not seen is one to commit rather than lose.
- **The skipped posts are in \`content/skipped-posts.md\`, and every line is a decision.** A skip records that somebody read a post and judged it does not belong in Quartz; nothing reproduces that, so losing the list means reading the whole blog again. The file holds the address and the title, and identity is the address compared the way the rest of the app compares one — /post/ and /reading/ are one Wix article — so a skip made from the feed is adopted rather than recorded twice. Applying re-skips anything the file names and the app has lost, which is the file winning as it always does: unskipping a post on Resources is followed by a commit, not left to the next sync.
- **Apply writes the file into the app; Commit writes the app into the branch.** Said that way round on the screen, in both buttons' titles, and here, because the sentence it used to carry — "Apply makes the file right" — meant Apply treats the file as the authority and read as Apply fixes the file. Somebody read it the second way and pressed the bulk Commit, and seven instruments reverted to wording the app had never been given.
- **Commit everything is the routine when the editing is done in the app, and it asks first.** One button beside Sync, one commit, every file whose app copy differs from its file — the subject names what changed and the body lists the paths. It names those paths before it writes them, and it holds back any file with writes outstanding: the app holds the older copy of exactly those files, so committing them is how a file loses an edit nobody applied. Those are named too, and their own row's Commit still offers it, which is where the decision belongs. The per-row Commit stays for the same reason — one row at a time is the right shape for deciding about a difference and the wrong shape for an ordinary afternoon, where keeping the repository current was four syncs and four commits and so got skipped. There is deliberately no Apply everything: applying overwrites what somebody typed into the app, and that is decided one file at a time after reading what changes.
- **System Health says whether the repository is still a backup.** Two checks read the content branch beside the app: *Content the repository does not have* is a fix, because nothing would bring that content back, and *Content that differs from the repository* is a judgement about which side is right. The comparison is the sync screen's own plan functions (\`content-status.js\`, pure; \`content-live.js\` does the reading), so a file that reads "up to date" on Content files cannot read as drift on Health. If the branch cannot be read, the reading throws rather than falling back to the copy bundled into the build, and both checks report unchecked — \`loadContent\`'s fallback is right for the sync screen, which says which copy it compared, and wrong for a check that has one line to say something with.
- **Instrument content is edited in two places, and neither is the master copy.** Questions, commentary, bands, and reading are edited on Settings → Instruments (InstrumentEditor), and equally in \`content/instruments/<key>.md\` on the \`content\` branch. Content files on that screen compares the two and names the field on the row that differs before anything is written; applying is one instrument at a time. Identity is a \`content_key\` that outlives a rename, so rewording a question is an edit to the row holding its answers rather than a second copy under the new name. A question the file no longer lists is reported and left alone — Response rows key on it — while a band is offered for removal, because one left behind still catches scores. The branch is not \`main\` deliberately: Base44 syncs main and only main, and a wording fix should not rebuild the app. Instrument questions are not library activities: isLibraryActivity keeps them out of the Library, activity sets, the assessment picker, the Resources picker, and any assessment that uses the whole library.
...
- Revising never un-completes a respondent. Every save during a revision carries completion, so the roster keeps them complete and completed_date tracks when their answers last changed. A revision abandoned halfway is still a whole set of answers, because every page saves as it is left.
- **Unread responses are per assessment, not "since you last logged in".** A session here lasts weeks, so login is a moment nobody remembers, and one global timestamp would clear every badge the instant the page loaded — exactly when you wanted to read them. Opening an assessment's Results is the act of reading, so that is what marks it seen. The first load stamps \`responses_seen_at.since\` to now, or every completion ever recorded would arrive as news. The field is deliberately self-writable with no per-field rule: it decides which badges a user sees and grants nothing.
- **The sidebar no longer lists assessments.** It grouped by client, owner, or organization, which gave most assessments a heading of their own in a column too narrow for their titles. The list is a table on the Assessments page now; the sidebar keeps Pinned, five Recent, and the ⌘K switcher, whose search uses the same every-word-anywhere rule as the page so the two boxes cannot disagree about what matches. **Mine** defaults on only when something is yours — a super-admin who created nothing otherwise opened on an empty page while the sidebar counted new responses.
- **A client is the Client company text, not a record.** The Assessments page's client dropdown groups by \`company_name\` trimmed and lower-cased, and shows the most-used spelling. It deliberately does no fuzzier matching — a near-duplicate belongs in the dropdown, where someone can see it and fix the name, not silently merged. The chosen client and Mine/Everyone's are held in \`AdminPage\`, so opening an assessment and coming back keeps them; neither survives a reload. Promoting Client to its own entity (contacts, report branding) is the larger version of this and has not been built.
- **Pins live on the user; Recent lives in the browser.** A pin is a deliberate choice, and making it again on a second device reads as the feature losing it; "what was I just in" is a question about this device. Recent leaves out anything pinned, because the same title twice in a short sidebar reads as two assessments. Pin controls sit on the assessment's header and its table row, never on the sidebar links, where a control on a one-line link is a mis-click. **New assessment is not in the sidebar**: it lives on the Assessments page beside the list it adds to, and in ⌘K from anywhere.
- **Every distribution bar says which side is which.** The bar is centred on the line rather than on itself, so a room leaning one way draws off to that side and read as a layout fault. The labels are worded for the scale (*Needs attention* / *In good shape*) rather than for Chaos's questions, because the same drawing serves all four instruments and left is always the lower-scoring answers.
- **A question keeps one number everywhere it appears.** \`surveyNumbers\` counts the rated questions in survey order, and the person's own summary, the team report, and the Discussion tab all read that number off it. The report and the discussion list are ordered by disagreement and reorder themselves as people finish, so a number counted off those lists named a different question on Wednesday than the copy printed on Monday — and never matched the copy in the respondent's hand, which carried no number at all. The numbers therefore do not run in order on either facilitator surface, and both say so, because "let's take number four" is the sentence the whole thing exists to make unambiguous. Written answers are not numbered: they are gathered under their own heading rather than discussed, and numbering them would push every other number out of step.
...
- **Every question states what it means, on every card.** \`PERSONAL_AXES\`, \`TEAM_GAP_AXES\`, and \`OWNERSHIP_QUESTION\` each carry a \`hint\`, and the intro screen renders its glossary from the same constants — so the screen describing the survey cannot describe it differently from the survey. The team gap's two axes were hardcoded JSX while the personal three came from a list, which is exactly how one of them would have been reworded and not the other. This is not decoration: a respondent who reads Experience and Skills as the same question rates them identically, which produces well-formed answers and empties the one category that needs those answers to disagree. Not a tooltip, because hover does not exist on the phones these are answered on; not once on the intro, because it is forgotten by the third activity. \`gray-500\`, not \`gray-400\` — that is a standing 2.43:1 contrast finding and this is text written for people who are unsure.
- **\`OWNERSHIP_QUESTION\` renders on exactly the condition the intro promises it on.** Both the intro and the activity card read \`!isPersonal && ownerOptions.length > 0\`, from one variable, so the screen describing the survey cannot promise a question the survey does not ask. Options come from \`ownerOptionsFor\` rather than from stored \`Assessment.roles\`, so in practice a team gap always asks it — the three product titles are in every survey — but the condition stays because a personal assessment never asks it and the coupling is the point.
- **\`IntroPurpose\` owns the whole intro block, including the confidentiality line.** That line used to be rendered by each of the two intro screens separately while the timing line sat inside the component, so the order of the two was something three files had to agree on and reversing them meant editing all three. Copy whose order carries meaning belongs in one component.
...
- **The ownership question asks who does the work today, not who should.** The library already carries the recommendation in each activity's \`preferred_owner\`, so asking a respondent to name it too collects an opinion we hold already; asking what actually happens supplies the other half, and the report can show recommended against actual. That gap is the coaching conversation — win/loss recommended to product marketing but owned by sales today is a finding, and it exists only if the survey asks about today. This reverses the earlier framing, which argued a respondent describing the status quo reports what the room already knows; the room knowing it is not the same as the report showing it. The stored field is still \`suggested_owner\` because renaming it would rewrite every answer already collected, but nothing user-facing calls it a suggestion: the column reads **Owns it today** and the tally **Most often named**. Nothing assigns anyone the job.
- **Titles are offered; functions are recommended; a product title satisfies either product function.** Every library activity recommends a *function* (\`Product Marketing\`), while respondents answer with a *title* — so \`ownership.js\` maps between them. The three product titles appear in every survey and displace \`Product Management\` and \`Product Marketing\` from the picker, because asking someone to choose between "Product Management" and "Product Manager / Product Owner" for one activity has no right answer and splits the tally between two spellings of one idea. The match is deliberately generous: any of the three satisfies either function. Customers disagree about what these words mean — Microsoft's "product manager" is what Quartz calls product marketing — so splitting them by title would report a difference in the customer's dictionary as a problem with their team, on every go-to-market activity. What the work *is* stays legible without the title: the activity names its recommended function and sits in a phase.`,
  },
];
