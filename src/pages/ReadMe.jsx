...
**Download content has to mean everything, and for a while it did not.** It exported a section as a bare name while the prose attached to it — a dimension's blurb, and the two paragraphs read when it is somebody's strongest or weakest — was left out entirely: ten of the practice profile's fifteen prose fields missing from its own backup, in a file that looked complete. Sections are objects now, carrying those keys where they exist. The instrument's own name, tagline and description are still absent on purpose, because Apply source rewrites them from the seed on every run and a copy here would be the stale one.
...
- **Reading lives at two addresses, and only one is the seed's.** Blog articles point at \`/reading/<slug>\`, the CTA-free copy the seed composes from \`ARTICLE_PREFIX\`. The 23 chaos limericks point at the Wix dynamic page \`/reading-idiom/<slug>\` (it was \`/i/<slug>\` until September 2026), were added in the app, and are unknown to the seed, so Apply source leaves them alone. Apostrophes are dropped from the slug (\`whos-on-first\`). Wix's \`dynamic-i_…-sitemap.xml\` may still list the old \`/i/\` addresses and apostrophe slugs, so check a page loads before trusting it.
- **Apply source never overwrites a library resource.** For an article already in Settings → Resources, the seeder sets only its address (the /reading/ policy) and merges in its question links, filling the note and author only when they are blank. Title, type, order, fallback, and active stay as the library set them, so an article switched off there stays off. A new article gets every field.
- **The app is the master copy of instrument content.** Questions, commentary, bands, and reading are edited on Settings → Instruments (InstrumentEditor). Apply source seeds an instrument only while it has no questions in the app; after that it never touches its questions, bands, or reading, because re-applying would revert edits and matching by label would turn a renamed question into a second copy. Download content is the backup version control used to be. Instrument questions are not library activities: isLibraryActivity keeps them out of the Library, activity sets, the assessment picker, the Resources picker, and any assessment that uses the whole library.
