...
\`publicAssessment\` resolves every public token server-side and returns only the fields that flow needs, plus \`org_name\` — the firm the printed report names, resolved from the assessment's organisation or its creator's, since the pages that render it are unauthenticated and cannot read \`Organization\` themselves · \`listRespondents\` and \`listUsers\` read as service role where RLS cannot express the rule · \`deleteAssessment\` cascades a delete after one authority check · \`deleteOrganization\`, \`deleteTeamMember\`, \`deleteTag\` and \`deleteLibraryActivity\` do the reverse, refusing while anything still references the row · \`listTags\` and \`listLibraryActivityUsage\` count usage across records the caller cannot read, so a Delete is only offered where it can work · \`acceptInvitation\` and \`updateTeamMember\` manage roles server-side — the latter refuses every self-edit except one, a super-admin setting their own organisation, whose access does not derive from it.`,
  },
  {
    id: "architecture",
    group: null,
...
**\`@page { margin: 0 }\` is a Chrome-only guarantee, and it was holding up a credential.** Zero margins suppress the browser's own header and footer, which is why the rule is there: every page that prints is token-authenticated, and on those pages the URL *is* the credential. Safari ignores it. A respondent's Safari-saved PDF came back with \`…/assess?t=<their resume token>\` stamped on all six sheets — a link that reopens and edits their answers, on the document they are invited to send to a manager. \`usePrintSafeUrl\` now reduces the address to the bare origin on \`beforeprint\` and restores it after, because the header shows whatever the address bar holds at the time. **Anything that both prints and carries a token in its URL needs that hook**, and CSS alone will not do it.
