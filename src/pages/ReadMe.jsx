...
| **User** | Base44 built-in, extended with \`org_id\`, an application \`role\`, and \`responses_seen_at\` — when this user last opened each assessment's Results, which drives the unread badges. |
...
| Route | Purpose |
|---|---|
| \`/admin\` | Everything the facilitator does. Opens on the Assessments page (\`AssessmentsHome\`) — a searchable table with response counts and unread badges — with a ⌘K switcher (\`AssessmentSwitcher\`) and a Recent list in the sidebar. Then setup, results, discussion, instruments, library, tags, organizations, and facilitators. The only route behind \`ProtectedRoute\`. |
