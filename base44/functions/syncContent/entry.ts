...
const ALLOWED = /^content\/(scales\.md|resources\.md|job-titles\.md|activity-sets\.md|instruments\/[a-z0-9][a-z0-9-]*\.md|library\/[a-z0-9][a-z0-9-]*\.md)$/;
  ...
  try {
    ...
    for (const f of files) {
      ...
      if (!ALLOWED.test(f.path)) {
        return fail(`${f.path} is not a content file. Only content/scales.md, content/resources.md, content/job-titles.md, content/activity-sets.md, content/instruments/<name>.md and content/library/<phase>.md can be written.`);
      }
